...
/* SVN FILE: $Id: PBMAssetManager.php 61 2010-04-16 10:19:59Z chris.l.yates $ */
/**
...
class PBMAssetManager extends CAssetManager
{
 ...
 /**
  * @var array Directories excluded from publishing
  */
  ...
  if(isset($this->_published[$path]))
		...
		else if(($src=realpath($path))!==false)
		{
			...
			else if(is_dir($src))
			{
				...
				$dstDir=$this->getBasePath().DIRECTORY_SEPARATOR.$dir;
                foreach($this->excludeDirectories as $e)
                {
                 $t = strpos(str_replace(Yii::getPathOfAlias('webroot'),'',$src),$e);
                    if($src===$e || strpos(str_replace(Yii::getPathOfAlias('webroot'),'',$src),$e)===0)
				...
				else if(!is_dir($dstDir) || $forceCopy)
				{
                    ...
                    foreach (CFileHelper::findFiles($dstDir, array('fileTypes' => array_keys($this->parsers), 'level' => $level)) as $file)
                    {
                     $fileName = basename($file);
                     $suffix = substr(strrchr($fileName, '.'), 1);
                     $dstDir = dirname($file);
                     //
                     $fileName = basename($file, $suffix) . $this->parsers[$suffix]['output'];
                     $dstFile = $dstDir . DIRECTORY_SEPARATOR . $fileName;
                     $parserClass = Yii::import($this->parsers[$suffix]['class']);
                     $parser = new $parserClass($this->parsers[$suffix]['options']);
                     file_put_contents($dstFile, $parser->parse($file));
                     unlink($file);
                    }
				}
 ...
 public function publishAdd($path, $hashByName=false, $level=-1)
 {
  if (isset($this->_published[$path]))
  {
   return $this->_published[$path];
  } else if (($src = realpath($path)) !== false)
  {
   if (is_file($src))
   {
    $this->publish($path, $hashByName, $level);
   } else if (is_dir($src))
   {
    $dir = $this->hash($hashByName ? basename($src) : dirname($src));
    $dstDir = $this->getBasePath() . DIRECTORY_SEPARATOR . $dir;
