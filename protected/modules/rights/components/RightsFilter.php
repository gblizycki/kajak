...
class RightsFilter extends CFilter
{
    ...
    public $checkPasswordDate;
    public $changePasswordUrl;
    public $generatePasswordUrl;
	/**
	...
	*/
	protected function preFilter($filterChain)
	{
		...
		{
            // Check password renewal date
            if($this->checkPasswordDate)
             $this->checkPasswordRenewalDate($filterChain);
	...
	*/
    ...
    public function checkPasswordRenewalDate($filterChain)
    {
     $change = is_array($this->changePasswordUrl)?CHtml::normalizeUrl($this->changePasswordUrl):$this->changePasswordUrl;
     $generate = is_array($this->generatePasswordUrl)?CHtml::normalizeUrl ($this->generatePasswordUrl):$this->generatePasswordUrl;
     $allowActions = array($change,$generate);
     $user = Yii::app()->getUser();
     $controller = $filterChain->controller;
     $action = $filterChain->action;
     $guest = $user->isGuest;
     if (!$user->isGuest && strtotime($user->passwordRenewal) < time() && in_array($controller->id . '/' . $action->id, $allowActions) === false)
     {
      $controller->redirect($this->changePasswordUrl);
     }
    }
}
